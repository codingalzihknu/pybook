...
# - 집합과 사전: 항목의 순서를 무시하고 중복을 허용하지 않는 두 개의 비순차 자료형
# - 형 변환 함수
# - `range` 객체
# - 리스트/사전 조건제시법
# - 모음 자료형과 `for` 반복문
...
# `hash()` 함수는 
# 인자로 지정된 객체에 대해 정수를 계산하여 반환한다.
# 이렇게 `hash()` 함수의 인자로 사용될 수 있는 값을
# **해시 가능**<font size="2">hashable</font>하다고 부른다.
# 해시 가능한 인자에 대해 `hash()` 함수가 값은 인자로 주어진 객체마다 유일하게 계산한다.
# 즉, 조금이라도 다른 객체가 지정되면 다른 값을 계산한다.
...
# 집합의 항목으로 해시 가능한 객체만 사용된다. 
# 리스트, 사전 등 가변 자료형의 값은 집합의 항목으로 사용될 수 없다.
# 
# ```python
# >>> {[1, 3], 4}
...
# 정수, 부동소수점, 문자열, 튜플 등 해시 가능한 자료형만 사전의 키로 사용될 수 있다.
