...
# 보다 복잡한 문제를 해결하는 프로그램을 하나 구현해보자.
# 
# 아래 코드는 **피보나치 수열**<font size="2">Fibonacci sequence</font>의
# 항목을 생성하는 코드를 구현해보자.
...
# 
# 위 알고리즘을 이용하여 피보나치 수열의 처음 10개 항목을 생성하는 코드는 다음과 같다.
...
# 위 코드에 사용된 각 명령문의 실행과정은 다음과 같다.
# 
# 1. `a, b = 0, 1`
#     - 변수 `a` 는 `0`을, 변수 `b` 에는 `1`을 할당한다.
# 1. `while a < 10:`
#     - 변수 `a` 가 가리키는 값이 `10` 보다 작은 동안 `while` 반복문의 본문 코드를 반복 실행하라.
#     - `while` 반복문의 본문은 바로 아래에 
#         **들여쓰기**<font size="2">indentation</font>로 구분된 코드를 가리킨다.
# 1. `while` 반복문 본문
#     - `print(a)`: 변수 `a` 가 현재 가리키는 값을 출력하라.
#     - `a, b = b, a+b`
#         - 변수 `a` 가 가리키는 값을 변수 `b` 가 이전에 가리키던 값으로 변경하라.
#         - 변수 `b` 가 가리키는 값을 변수 `a` 와 변수 `b` 가 이전에 가리키던 값의 합으로 변경하라.
