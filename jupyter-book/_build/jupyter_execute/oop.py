...
# :class: info
# 
# 클래스는 객체를 생성할 때 사용되는 설계도라고 이해할 수 있다.
# 클래스의 객체는 설계도에 묘사된 상태를 띄게 되며,
# 설계도에서 구현된 기능을 갖는다.
# 
# 예를 들어, `up2three` 변수는 집합 `{1, 2, 3}` 을 가리킨다.
# 
# ```python
# up2three = set([1, 2, 3])
# ```
# 
# 이를 달리 표현하면, `up2three` 는 `set` 클래스의 객체인 `{1, 2, 3}` 를 가리키는 변수다.
# 이처럼 클래스를 함수 호출하는 방식과 유사하게 이용하여 해당 클래스의 객체를 생성한다.
# 집합을 포함하여 지금까지 다뤄온 리스트, 사전, 정수, 부동소수점, 부울값 등 모든 값은
# 해당 자료형 클래스의 객체다. 
# 
# 집합 객체의 상태는 예를 들어, 집합의 원소, 원소의 개수 등을 가리키며,
# 집합의 기능은 `union()`, `add()` 등 {ref}`집합 자료형의 다양한 메서드 <sec:set-methods>`로 지정된다.
# :::
...
# :class: info
# 
# 아래 코드는 생성된 `Sprite` 객체를 `wizard` 변수가 가리키도록 한다.
# 
# ```python
# wizard = Sprite(-128, 200, 128, 128, "wizard.gif")
# ```
# 
# 즉, 아래 코드가 마치 함수 호출처럼 실행되면 `Sprite` 클래스의 객체가 하나 생성된다.
# 
# ```python
# Sprite(-128, 200, 128, 128, "wizard.gif")
# ```
# 
# 실제로는 `Sprite` 클래스의 `__init__()` 메서드가 아래 방식으로 적절한 인자들과 함께 호출된다. 
# `self` 매개변수에 해당하는 인자는 생성되는 객체가 사용된다. 
# 
# ```python
...
# 즉, 두 객체 사이에 주고 받을 수 있는 정보를 이용한다. 
# 
# 이처럼 객체들 사이에 정보를 주고 받을 수 있는 정보를 이용하여 객체의 상태와 행동을
