...
# 특정 명령문을 자동으로 반복<font size="2">iteration</font> 실행하는 장치인 **반복문**을 소개한다.
...
# 두 값의 등가 여부가 아니라 왼편에 위치한 변수가 오른 편에 위치한 값을 가리키도록 하는
...
# **간편 변수 업데이트**
# 
# `x = x+1`, `x = 2 * x` 등과 같은 간단한 변수 업데이트는 보다 `x += 1`, `x *= 2` 등으로 
# 보다 간편하게 나타낼 수 있다. 
# 
# |연산 기호|예시|의미|
# |:----------:|:----------:|:----------:|
...
# 헤더의 `부울식`이 계속해서 참이면 `while` 반복문의 본문이 무한히 반복되어 실행된다.
...
# 실행될 명령문이 없다.
# 따라서 `count_infinitely(1)`을 호출하면 `while` 반복문이 무한 반복되며
...
# `x == y` 또는 `x != y` 처럼 
# 부동소수점<font size="2">floating point</font>의 등가성을 
