...
# **컴퓨터 프로그램**(computer program)이란, 어떤 주어진 문제를 해결하기 위해서 특정 프로그래밍 언어를 이용하여 컴퓨터가 수행해야 할 작업들을 모아 놓은 명령문의 모음이라고 볼 수 있다. 
# 
...
# 사용되곤 한다. 
# 네 개념을 엄밀히 구분하는 기준은 없다. 
# 하지만 일반적인 용도를 살펴 보았을 때 다음과 같이 구분할 수 있다.
# 
# - 소프트웨어<font size="2">software</font>: 하드웨어를 이용하여 특정 기능을 수행하도록 하는 컴퓨터 프로그램 일반을 가리킴.
# - 프로그램<font size="2">program</font>: 컴퓨터에서 실행할 수 있는 하나의 소프트웨어 제품.
# - 소스 코드<font size="2">source code</font>: 프로그램 구현에 사용된 명령문 모음집. 
#     원시 코드라고도 불림.
# - 코드<font size="2">code</font>: 소스 코드의 일부분 또는 전체.
# - 명령문<font size="2">statements 또는 commands</font>: 소스 코드의 구성요소
# ::::

# **프로그래밍**(programming)은 특정 목적을 달성하기 위해 컴퓨터가 실행할 수 있는 컴퓨터 프로그램을 디자인 및 구현하는 과정을 말한다. 

# ::::{admonition} 프로그래밍과 코딩
# :class: tip
# 
# 일상적으로 프로그래밍<font size="2">programming</font>과 
# 코딩<font size="2">coding</font>이 동의어로 사용된다.
# 다만 프로그래밍은 프로그래머<font size="2">programmer</font>의 직업과 관련된 개념으로
# 코딩은 "코드를 작성한다"의 의미로 보다 많이 사용된다.
# ::::

# **프로그래밍 언어**(programming language)는 컴퓨터 프로그램을 만들 때 사용하는 언어를 말한다. 자연어와 마찬가지로 다양한 프로그래밍 언어가 있으며, 일반적으로 저수준 언어(low-level language)와 고수준 언어(high-level language)로 나눈다.
# 
...
# 
# - 고수준 언어는 사람이 이해하기 쉽게 작성된 프로그래밍 언어로, 
...
#     대부분의 프로그래밍 언어들이 고수준 언어다. 
#     고수준 언어는 저수준 언어보다 가독성이 높고 사용이 편리하여 
#     프로그래머들은 주로 고수준 언어를 사용한다. 
#     반면, 컴퓨터는 기계어만 이해할 수 있기 때문에, 
#     고수준 언어로 만들어진 프로그램은 기계어로 변경해야만 컴퓨터에서 실행할 수 있다. 
#     이때 컴파일러(compiler) 또는 인터프리터(interpreter)를 사용한다. 
