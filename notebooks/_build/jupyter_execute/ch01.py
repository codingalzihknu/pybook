...
# <figcaption>하드웨어와 소프트웨어</figcaption>
...
# 참고) 원시코드 또는 소스코드(source code)는 특정 프로그래밍 언어를 이용하여 작성한 프로그램을 말한다. 
...
# 참고) 목적 코드 또는 오브젝트 코드(object code)는 어셈블러나 컴파일러가 소스 코드를 어셈블 또는 컴파일하여 만든 코드이다.
...
# 
# - 컴파일러는 특정 프로그래밍 언어를 다른 프로그래밍 언어로 변환하는 언어 번역 프로그램을 말하며, 변환하는 과정을 컴파일(compile)이라 부른다. 주로 고수준 언어를 컴퓨터가 이해할 수 있는 기계어로 변경하는 데 사용된다. 컴파일과정은 아래 그림과 같다. 
#  
# 그림과 같이 컴파일러 방식에서는 컴파일러로 소스코드 전체를 훑은 다음, 한꺼번에 오브젝트코드로 변환하고 이를 일괄 실행한다.  C언어나 자바 등과 같은 대부분의 고수준 언어가 컴파일러 방식을 사용하고 있다. 
# 
# - 컴파일러와 달리 인터프리터는 소스코드를 한 줄씩 해석하여 바로 실행되게 하는 프로그램을 말한다. 인터프리터 작동 방식은 아래 그림과 같다. 
# 
# 
...
# 참고) 프로그램, 코드, 명령문, 소스코드
# **프로그램(완성된 형태), 코드(소스코드의 일부분), 명령문(한 줄 한 줄), 소스코드..
# **프로그래밍과 코딩

# ## 파이썬이란

# 파이썬(Python)은 1991년 귀도 반 로섬(Guido van Rossum)이 개발한 고수준 프로그래밍 언어로, 거의 모든 운영체제에서 동작하는 인터프리터 방식의 범용 프로그래밍 언어(general-purpose programming language)이다.
# 
# 기존 프로그래밍 언어들의 한계를 극복하고자 만들어진 파이썬은 자연어와 유사할 정도로 간결하고 쉬운 문법을 가지고 있다. 예를 들어, 아래의 파이썬 코드는 you가 members에 없다면 회원가입을 하라는 문구를 출력하는 코드이다. 
...
# 위의 파이썬 코드는 프로그래밍에 대한 경험이 전혀 없는 사람도 대략적으로 의미를 유추하는 것이 가능할 정도로 자연어와 유사하다. 파이썬은 들여쓰기로 코드 블록을 구분하는 데, 이는 코드의 가독성을 높인다. 또, 파이썬의 간결하고 쉬운 문법은 오류가 발생할 가능성을 낮춰 프로그램 개발 속도와 생산성을 높일 수 있다.
# 
# 이에 구글Google, 드롭박스Dropbox, 인스타그램Instagram 등 많은 회사에서 소프트웨어 개발에 파이썬을 사용하고 있다. 반면, 실무에서 사용하는 프로그래밍 언어는 다루기 어려워서 보통 프로그래밍 입문자에게 추천하지 않는데, 파이썬은 상대적으로 배우기 쉬운 프로그래밍 언어라 실무에서뿐만 아니라 입문자 교육용으로도 많이 사용되고 있다. 

# 참고) 귀도 반 로섬은 어느 인터뷰에서 당시 프로그램 개발에 사용했던 C언어의 생산성이 낮아 새로운 언어의 필요성을 느꼈다고 말하였다. 
...
# 파이썬은 C언어나 자바 등과 같이 범용 프로그래밍 언어로, 데이터 분석, 머신러닝, 데이터베이스 관리, 웹 개발, 웹 스크래핑, GUI 프로그래밍(Graphical User Interface programming) 등 다양한 용도로 사용할 수 있다. 
# 
...
# 한편, 파이썬은 C++이나 자바 등과 같은 객체 지향 언어(Object-Oriented Programming, OOP)이다. 파이썬은 모든 것이 객체로 잘 설계되어 있어서 매우 편리하고 강력한 기능을 제공한다. 

# 참고) OOP는 구현해야 할 객체들을 선택하고, 객체들 사이의 유기적인 관계를 논리적으로 묘사하는 것을 중요하게 여기는 프로그래밍 기법으로, 프로그램 전체를 하나로 묶어서 구현하는 것이 아니라 프로그램을 구성하는 객체들을 중심으로 하여 구현해야 할 프로그램을 완성시키는 방식으로 프로그래밍을 진행하는 것을 말한다. OOP를 잘 활용하면 코드의 재사용성을 높이고, 유지보수하기 좋은 코드를 작성할 수 있다.

# 참고) 보통 OOP와 대비하여 절차 지향 프로그래밍(Procedural Programming)을 언급한다. 절차 지향 프로그래밍에서는 수행해야 할 일을 순차적으로 처리하는 과정을 묘사하는 것을 중요하게 여기며, 프로그램 전체가 유기적으로 연결되도록 만드는 프로그래밍 기법이다. 대표적인 절차 지향 프로그래밍 언어로는 C와 HTML이 있다.  

# 또, 파이썬은 동적 타이핑(Dynamic typing)을 지원한다. 동적 타이핑은 변수를 미리 선언하지 않고, 변수를 생성하고자 할 때 값을 초기화하여 사용하는 것을 말한다. 변수의 자료형은 파이썬이 알아서 판단하기 때문에 동적 타이핑은 프로그래밍을 처음 접하는 사람들에게 편리하다. 

# 참고) 동적 타이핑과는 달리 변수를 사용하기 전 미리 선언해야 하는 정적 타이핑(Static Typing)도 있다. C나 자바처럼 주로 컴파일 방식의 언어가 정적 타이핑을 지원한다. 이처럼 사용할 변수와 변수에 할당될 값의 자료형을 미리 선언하는 것은 불편해 보일 수도 있다. 하지만 정적 타이핑은 컴파일하는 과정에서 선언한 자료형에 맞는 값을 할당하였는지, 선언 후 사용하지 않는 변수가 있는지를 확인하여 버그 발생 확률을 낮출 수 있다는 장점이 있다. 이에 동적 타이핑을 지원하는 파이썬에서도 타입 힌트를 명시하기도 한다. 
...
# 지금까지 살펴본 파이썬의 특징으로 인해  파이썬의 인기는 날이 갈수록 높아지고 있다. 프로그래밍 언어들의 인기도를 측정하는 TIOBE index(https://www.tiobe.com/tiobe-index//)에 따르면, 2022년 1월 기준으로 파이썬은 가장 인기있는 프로그래밍 언어이다. 
