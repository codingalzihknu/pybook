...
# 그런데 변수는 가리키는 값을 변경할 수 있다.
# 예를 들어, 변수 `num`에 18을 새롭게 할당하면
# 이제는 17이 아닌 18을 가리킨다.
...
# 변수의 이름을 지정할 때 할당되는 값과 연관된 이름을 사용할 것을 권장한다.
...
# 파이썬의 경우 대문자로 시작하는 이름은 주로 나중에 배울 클래스를 선언할 때 사용한다.

# **한글 사용**

# 한글 단어, 심지어 자음, 모음도 독립적으로 사용할 수 있다.
...
# '안녕하세요!', 18, 3.1415926535897932 등은 저장하거나 
# 다른 연산에 사용할 수 있는 값들이다. 
# 반면에 `num` 이 하나의 값을 가리키면 `num + 2`도 하나의 값을 가리킨다.
# 이렇게 변수, 덧셈 연산, 정수 등을 포함해서 
# 하나의 수(number)를 표현하는 수식이 **표현식**이다.
# 하지만 문자열 연산을 나타내는 `'hello' + '!'` 처럼 수가 아닌 다른 종류의 값도 
# 표현식으로 나타내도록 할 수 있다. 
...
# 연산 개념과 동일하다.
# 하지만 수가 아닌 다른 종류의 값들을 대상으로하면 의미가 달라질 수 있다.
...
# 문자열을 정수와 곱하면 해당 문자열을 정수만큼 복제하여 모두 연결한다.
...
# 함수는 기본적으로 수학에서 다루는 다음과 같은 함수를 가리키며, 
# 정수, 유리수, 문자열 등을 연산자, 함수 등과 조합하여
# 특정 값을 나타내도록 하는 식이 바로 **표현식**이다. 
...
# 그런데 `print()` 함수의 경우처럼 일반 수학 함수와는 다른 종류의 함수도
# 프로그래밍에서 사용된다.
# 파이썬에서 함수를 어떻게 정의하고 활용하는가는 이후에 자세히 다룬다.
