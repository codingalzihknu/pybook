...
# 아직은 일반적이지 않으며, 영어 등 라틴어 계열의 단어를 주로 사용한다.
...
# 다만, 여기서는 앞서 설명한 연산자들 또한 함수로 간주된다는 점과
...
# 언어의 특성에 따라 명령문 작성법과 작성된 명령문의 모양이 크게 다를 수 있다.
# 
# 파이썬 명령문은 **기본 명령문**들을 지정된 규칙에 따라 조합해서 생성한다.
# 지금까지 간략하게나마 살펴본 기본 명령문들은 다음과 같다.
# 
# * 변수 할당
# * 조건 제어문
# * `while` 반복문
...
# 이와 같이 프로그램의 실행과는 무관하지만 명령문을 기능, 아이디어 등을 
# 설명하는 문장을 
# **주석**<font size="2">comment</font>이라 한다.
# 주석은 일반 자연어를 사용하며,
...
# 변수 이름을 잘못 지정하는 경우 `SyntaxError: invalid syntax`와 
...
# 프로그램 실행 중간에 예외적인 상황을 발생시키는 오류라는 의미에서 런타임 오류를 
...
# 프로그램이 실행은 잘 되지만 원하는 결과를 가져오지 못한다면 프로그램
# 어딘가에 논리적 오류가 존재한다는 의미이다.
# 이와같은 오류를 
# **의미 오류**<font size="2">semantic error</font>라 부르며,
# 발생원인은 매우 다양하다.
# 실제로 프로그램은 작동하기 때문에 구문 오류나 런타임 오류와는 다르게
# 의미 오류의 원인을 바로 확인하기 어려울 수 있다.
# 
...
#     1. 반지름이 r인 구의 부피는 $\frac{4}{3} \pi r^3$ 이다. 반지름이 7인 구의 부피는 얼마인가? 
#         
