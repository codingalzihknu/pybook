...
# 아래 식에서 사용되는 `==`는 양편에 위치한 두 값의 동치(equality) 여부를 판단한다.
...
# :class: caution
# 
# 프로그램을 작성할 때 한글을 변수 등의 이름으로 사용하는 일은 아직 거의 없으며,
# 전통적으로 영어 등 라틴어 계열의 단어를 사용한다.
# 하지만 변수가 가리키는 값으로는 한글 등 다양한 언어가 사용된다.
# ```
