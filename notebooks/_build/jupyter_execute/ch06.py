...
# ::::{prf:example}
# :label: exp:first-class-function
# 
# `do_twice()`는 인자 없는 함수 하나를 입력 받아
# 해당 함수를 두 번 호출한다.
# 
# ```python
# def do_twice(f):
...
# ```
# 
# 이제 `print_spam()` 함수를 두 번 호출해서 `'스팸'` 단어를
# 두 번 출력하기 위해 `do_twice()` 함수를 호출할 수 있다.
# 
# ```python
# >>> do_twice(print_spam)
# 스팸
...
# ```
# 
# 아래 조건을 만족하도록 `do_twice()` 함수를 수정해보자.
# 
# * 두 개의 인자를 받는다.
# * 첫째 인자는 하나의 인자를 받는 함수가 사용된다.
# * 둘째 인자는 첫째 인자로 사용된 함수의 인자로 사용될 값이다. 
# * 첫째 인자로 들어온 함수를 둘째 인자로 들어온 값과 함께 2번 연속 호출한다. 
# 
# 다음과 같이 할 수 있다.
# 
# ```python
...
# 
# 
# 아래 조건을 만족하도록 `do_four()` 함수를 정의해보자.
# 
# * 두 개의 인자를 받는다.
# * 첫째 인자는 하나의 인자를 받는 함수가 사용된다.
# * 둘째 인자는 첫째 인자로 사용된 함수의 인자로 사용될 값이다. 
# * 첫째 인자로 들어온 함수를 둘째 인자로 들어온 값을 이용하여 4번 연속 호출한다. 
# * 앞서 정의한 `do_twice()` 함수를 반드시 활용한다.
# 
# ```python
