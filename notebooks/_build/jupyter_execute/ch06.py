...
# ```
# 
# 둘째 매개 변수 자리에 위치한 `...`은 화면에 출력할 여러 개의 값이 인자로 사용될 수 있음을 의미한다.
# `end='\n'` 매개 변수는 지정된 인자들을 출력한 후에 기본적으로 줄바꿈을 실행하는 것을 의미한다.
# `end=''`로 설정하면 줄바꿈을 하지 않게 된다. 아래 두 경우를 비교하면 차이점을 확인할 수 있다.
# 
...
# :::{admonition} 빈 문자열 `''` 과 공백 문자 `' '`
# :class: info
# 
# 빈 문자열<font size="2">empty string</font> `''` 는 
# 아무런 문자를 포함하지 않으며 따라서 문자열의 길이가 0이다.
...
# 반환값을 다른 프레임에 넘겨 준 후에 사라지는 과정이 반복된다. 이렇게 특정 함수 호출과 관련된 프레임을 **지역 프레임**이라 
# 부른다.
